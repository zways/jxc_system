    ...
    <div class="inbound-record-container">
        <div class="page-header">
            ...
            <div class="header-actions">
                ...
                <el-button type="success" :icon="Plus" @click="openAdd">新增入库单</el-button>
            </div>
        </div>
        ...
        <el-dialog v-model="dialogVisible" :title="dialogTitle" width="560px" :close-on-click-modal="false" :trap-focus="false" @open="formRef?.clearValidate?.()" @opened="onInboundDialogOpened">
            <el-form :model="form" :rules="formRules" ref="formRef" label-width="100px">
                ...
                <el-form-item v-if="selectedProduct?.track_batch" label="批次号" prop="batch_number">
                    <el-input v-model="form.batch_number" :disabled="isViewMode" placeholder="必填" clearable />
                </el-form-item>
                <el-form-item v-if="selectedProduct?.track_batch" label="生产日期" prop="production_date">
                    <el-date-picker v-model="form.production_date" :disabled="isViewMode" type="date" value-format="YYYY-MM-DD" placeholder="可选" style="width: 100%" />
                </el-form-item>
                <el-form-item v-if="selectedProduct?.track_batch" label="有效期至" prop="expiry_date">
                    <el-date-picker v-model="form.expiry_date" :disabled="isViewMode" type="date" value-format="YYYY-MM-DD" placeholder="可选" style="width: 100%" />
                </el-form-item>
                <template v-if="selectedProduct?.track_serial">
                    <el-form-item v-for="(_, idx) in form.serial_numbers" :key="idx" :label="`序列号 ${idx + 1}`" :prop="'serial_numbers.' + idx">
                        <el-input v-model="form.serial_numbers[idx]" :disabled="isViewMode" placeholder="必填" clearable />
                    </el-form-item>
                </template>
                ...
                <el-form-item label="关联单据" prop="reference_document">
                    <el-input v-model="form.reference_document" :disabled="isViewMode" placeholder="可选，如：PO202602070001" />
                </el-form-item>
...
export default defineComponent({
    ...
    setup() {
        ...
        const onQuantityChange = () => {
            if (!selectedProduct.value?.track_serial) return;
            const n = Math.max(0, parseInt(form.quantity, 10) || 0);
            const prev = form.serial_numbers.slice(0, n);
            while (prev.length < n) prev.push("");
            form.serial_numbers = prev;
        };
        ...
        const viewDetail = (row) => {
            ...
            form.production_date = row.production_date ? (typeof row.production_date === "string" ? row.production_date : row.production_date?.slice?.(0, 10)) : null;
            form.expiry_date = row.expiry_date ? (typeof row.expiry_date === "string" ? row.expiry_date : row.expiry_date?.slice?.(0, 10)) : null;
        ...
        const removeRow = async (row) => {
            try {
                await ElMessageBox.confirm(`确定删除该入库记录（${row.transaction_number}）吗？`, "删除确认", {
                    type: "warning",
        ...
        const submitForm = async () => {
            ...
            if (selectedProduct.value?.track_serial) {
                const valid = form.serial_numbers.filter((s) => String(s || "").trim()).length === form.quantity;
                if (!valid) {
                    ElMessage.warning("请填写与数量一致的序列号（每行一个）");
                    return;
                }
            }
            try {
            ...
            try {
                ...
                if (selectedProduct.value?.track_batch) {
                    payload.batch_number = form.batch_number?.trim() || null;
                    payload.production_date = form.production_date || null;
                    payload.expiry_date = form.expiry_date || null;
                }
                if (selectedProduct.value?.track_serial) {
                    payload.serial_numbers = form.serial_numbers.map((s) => String(s || "").trim()).filter(Boolean);
                    payload.quantity = payload.serial_numbers.length;
                }
                if (isEdit.value && form.id) {
                    if (payload.serial_numbers) {
                        delete payload.serial_numbers;
                        payload.serial_number = form.serial_numbers[0]?.trim() || null;
                    }
